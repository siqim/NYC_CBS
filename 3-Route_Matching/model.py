...
def optim(num_available_buses):
    ...
    with open('./solutions/solution_%d.xml'%num_available_buses, 'w') as f:
        f.write(solution.export_as_mst_string())

    with open('./solutions/solution_%d.pk'%num_available_buses, 'wb') as f:
        sol = {'y': solution.get_value_dict(y), 'obj': solution.objective_value}
        pickle.dump(sol, f)
...
#sns.set()
#
#rev = [299.229, 566.211, 784.449, 959.355, 1124.62, 1242.22, 1335.41, 1426.41, 1508.84, 1587.84, 1664.84, 1693.84, 1718.94]
#y = np.arange(1, 14)
#
##sns.lineplot(y, rev)
#
#sns.regplot(x=y, y=rev,x_estimator=np.mean, logx=True, truncate=True)
#plt.title('Profit v.s. Number of Buses')
